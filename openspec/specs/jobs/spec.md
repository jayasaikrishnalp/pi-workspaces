...
Persistent unit of agent work, SQLite-backed. One Job is created per chat send (lifecycle hooked from the pi-rpc-bridge). Status transitions: queued → running → completed | failed | cancelled. Stays the system of record across server restarts. The frontend's Jobs view reads /api/jobs.
