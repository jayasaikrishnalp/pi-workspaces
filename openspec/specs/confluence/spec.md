...
Owns the workspace's outbound Confluence integration: search and page fetch against the allowlisted Atlassian site. Implements the 10 hardening items from locked spec §Stage 5: allowlist, validation, error redaction, prompt-injection markers, env-driven auth, timeouts, CQL safety, input clamps, sanitize-html, normalized errors + cache.
