...
MCP (Model Context Protocol) client pool exposed by the cloudops-workspace backend. Hosts long-lived stdio + HTTP MCP server connections, surfaces a flat tool list, forwards tool calls with bounded timeouts, and ships a thin pi extension that registers each MCP tool natively in pi's tool-use loop. Centralized here (rather than inside pi) so the workspace owns config, lifecycle, audit, and a future Settings UI.
