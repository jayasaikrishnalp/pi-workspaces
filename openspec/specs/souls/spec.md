...
Soul = the agent's character / identity definition. File-based at <kbRoot>/souls/<name>/SOUL.md, with frontmatter capturing values, priorities, decision principles, tone, and an optional model preference. Reusable: any number of agents can reference the same soul via their optional soul: frontmatter field. Souls become graph nodes (source: "soul") with embodies edges from referencing agents.
