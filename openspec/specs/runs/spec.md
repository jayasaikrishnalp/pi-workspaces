...
Owns persistent per-run state: monotonic seq, eventId, events.jsonl + meta.json on disk under ~/.pi-workspace/runs/<runId>/, and the replay-aware GET /api/runs/:runId/events SSE channel. Disk is the source of truth; the chat-event-bus is a strict suffix of disk.
