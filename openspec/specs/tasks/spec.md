...
Operator + agent todos, SQLite-backed. Hermes-kanban shape (triage → todo → ready → running → blocked | done | archived). source: 'operator' | 'agent' distinguishes who created the task. State-machine guard rejects illegal transitions with 409 INVALID_TRANSITION. Idempotency-key dedup prevents duplicate creation under retry.
