...
Capability matrix exposed at GET /api/probe — pi reachability (real spawn of `pi --version`), Confluence configuration, skill/agent/workflow/memory counts, pi auth.json presence, active provider/model. Cookie-gated. The frontend uses this to render an honest startup screen.
