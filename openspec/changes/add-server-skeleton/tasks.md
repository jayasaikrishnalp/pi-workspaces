...
- [ ] 1.3 Sync the OpenSpec structure from Mac (`openspec/AGENTS.md`, `openspec/specs/` empty, `openspec/changes/add-server-skeleton/` with all 4+2 artifacts) → commit as `openspec: bootstrap + propose add-server-skeleton`
...
- [ ] 3.7 Install `SIGTERM`/`SIGINT` handlers — close server, force `process.exit(0)` after 5s
...
- [ ] 4.1 `server: boots on configured port` — start with PORT=0, read bound port, assert listening
- [ ] 4.2 `server: default port when PORT unset` — start with PORT unset, expect 8766 (skip if 8766 occupied; document why)
- [ ] 4.3 `server: port collision exits non-zero with EADDRINUSE` — pre-bind 8766, spawn server, expect exit !=0 and stderr contains "EADDRINUSE"
- [ ] 4.4 `server: SIGTERM exits cleanly within 5s` — start, send SIGTERM, expect exit code 0 within 5s
- [ ] 4.5 `server: SIGINT exits cleanly within 5s` — start, send SIGINT, expect exit code 0 within 5s
- [ ] 4.6 `server: unknown path returns structured 404` — `GET /api/does-not-exist` → 200 status check FAILS, expect 404 JSON shape, Content-Type=application/json

### 4b. health-domain scenarios → tests/health.smoke.mjs

- [ ] 4.7 `health: healthy response shape` — `GET /api/health` → 200, body `{ok:true, version:<semver>}`, Content-Type=application/json
- [ ] 4.8 `health: endpoint requires no authentication` — `GET /api/health` with no Cookie/Authorization → 200 (not 401)
- [ ] 4.9 `health: wrong method returns 405` — `POST /api/health` → 405, Allow: GET, body matches METHOD_NOT_ALLOWED shape

## 5. Documentation

- [ ] 5.1 `README.md`: prerequisites (Node ≥22), install (`npm i`), run (`./start.sh` or `npm run dev`), test (`npm run test:smoke`), env vars (`PORT`)
...
- [ ] 7.1 Generate test data summary in plain English (per locked-spec §8): annotated curl outputs + test pass/fail table
...
- [ ] 8.3 Move `openspec/changes/add-server-skeleton/` → `openspec/changes/archive/2026-05-05-add-server-skeleton/`
